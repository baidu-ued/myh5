...
<section class="box-wrap">
    ...
    <div class="ooooo" v-if="currentItemId != -1">
		...
		<div class="public-color-wrap">
            <button @click="changeStyle({ 'fontStyle' : items.css('fontStyle') == 'italic' ? 'normal' : 'italic'})">倾斜</button>
        </div>

        <div class="public-color-wrap">
            <p>宽度</p>
            <input class="public-text-input" type="number" :value="parseInt(currentItem.style.width) || parseInt(items.css('width'))" @input="changeStyle({ 'width' : $event.target.value + 'px'})">px
        </div>
        <div class="public-color-wrap">
            <p>高度</p>
