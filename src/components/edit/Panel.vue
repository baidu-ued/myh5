...
<section class="box-wrap">
    <div class="box">
        ...
        {{currentItem}}
        <ul class="color-wrap">
            <li @click="changeMain(['background', item])" class="color-item" v-for="item in bgColor" :style="{background: item}"></li>
        </ul>
        ...
        <div v-if="currentItemId != -1">
            <p>内容</p><input type="text" v-model="currentItem.content">
            <p>文字颜色</p><input type="color" v-model="currentItem.style.color">
			<p>背景颜色</p>{{currentItem.style.background}}<input type="color"  v-model.lazy="currentItem.style.background">
        </div>
    </div>
</section>
...
export default {
    ...
    computed: {
		...
		vals : function(){
			return this.currentItem.style.background || '#ffffff';
		}
    },
    methods: {
        ...mapActions(['changeMain', 'changeStyle']),
		aaa : function(val){
			this.changeStyle({'background' :  val})
		}
    },
    data: function() {
        return {
